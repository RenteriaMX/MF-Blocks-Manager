...
detect_nginx_config() {
    ...
    # Multi-tenant: un server block por slug en conf.d/<slug>.conf, donde el slug
    # es el nombre del dir del proyecto (ej. /home/bernuy/ama -> conf.d/ama.conf).
    # Es la convencion de la plataforma Plone MF actual (WAF compartido por slug).
    if [ -z "$NGINX_CONF" ]; then
        slug=$(basename "$PROJECT_DIR")
        if [ -f "/etc/nginx/conf.d/${slug}.conf" ]; then
            NGINX_CONF="/etc/nginx/conf.d/${slug}.conf"
        fi
    fi

    # Ultimo recurso: cualquier config que delate un site Plone/Volto. Ademas de
    # proxy_pass a volto/plone, reconocemos "location /static/mf" (remotes de
    # Module Federation), presente en el server block de la plataforma MF.
    if [ -z "$NGINX_CONF" ]; then
        NGINX_CONF=$(grep -rl "proxy_pass.*volto\|proxy_pass.*plone\|location /static/mf" /etc/nginx/ 2>/dev/null | head -1)
    fi
...
setup_nginx() {
    ...
    # Marca si algun paso requirio intervencion manual, para no reportar
    # una instalacion "exitosa" cuando Nginx quedo configurado a medias.
    NGINX_NEEDS_ATTENTION=false
    ...
    if [ -z "$NGINX_CONF" ]; then
        ...
        echo "    }"
        echo ""
        NGINX_NEEDS_ATTENTION=true
        return
    fi

    # ¿El config detectado tiene un dir de drop-ins "<base>.d/"? (p.ej.
    # /etc/nginx/conf.d/ama.conf -> /etc/nginx/conf.d/ama.d/). Si existe, escribimos la
    # location como ARCHIVO SEPARADO ahi: SOBREVIVE a re-generaciones del config principal
    # (Ansible u otro regenera ama.conf sin pisar el .d). Si no existe, la insertamos inline.
    SNIPPET_DIR="${NGINX_CONF%.conf}.d"
    if [ -d "$SNIPPET_DIR" ]; then
        SNIPPET="$SNIPPET_DIR/mf-blocks.conf"
        if [ -f "$SNIPPET" ] && grep -qE "location[[:space:]]+/mf-blocks/" "$SNIPPET"; then
            log "location /mf-blocks/ ya existe (drop-in $SNIPPET)."
        else
            log "Escribiendo location /mf-blocks/ en drop-in $SNIPPET..."
...
NGX
            if [ -w "$SNIPPET_DIR" ]; then
                cp "$TEMP_DIR/mf-blocks.conf" "$SNIPPET" || { error "No se pudo escribir $SNIPPET"; }
            else
                log "El dir es de root -> se escribe con sudo (te pedira tu password)."
                sudo cp "$TEMP_DIR/mf-blocks.conf" "$SNIPPET" || { error "No se pudo escribir $SNIPPET con sudo"; }
            fi
        fi

    # Sin dir de drop-ins: idempotencia por la DIRECTIVA real "location /mf-blocks/" (NO el
    # substring "mf-blocks", que puede salir en comentarios y dar falso positivo -> 404).
    elif grep -qE "location[[:space:]]+/mf-blocks/" "$NGINX_CONF"; then
        log "location /mf-blocks/ ya existe en Nginx."
    else
        log "Agregando location /mf-blocks/ a $NGINX_CONF (inline)..."

        # RESPETAR el archivo: solo INSERTAR el location (antes del primer "location /"),
        # sin tocar el resto. python3 transforma a un TEMP; se coloca con sudo si es de root.
        NEW_CONF="$TEMP_DIR/nginx-mfblocks.conf"
        python3 -c "
import re
...
with open('$NEW_CONF', 'w') as f:
    f.write(content)
print('OK')
" || { error "Error al generar la nueva config para $NGINX_CONF"; }

        if [ -w "$NGINX_CONF" ]; then
            cp "$NEW_CONF" "$NGINX_CONF" || { error "No se pudo escribir $NGINX_CONF"; }
        else
            log "El archivo es de root -> se actualiza con sudo (te pedira tu password)."
            sudo cp "$NEW_CONF" "$NGINX_CONF" || { error "No se pudo escribir $NGINX_CONF con sudo"; }
        fi
    fi

    # Detectar el usuario con el que corren los workers de nginx: el no-root
    # mas frecuente en la tabla de procesos (mas robusto que tomar el primero
    # alfabeticamente cuando coexisten varias instancias de nginx).
    NGINX_USER=$(ps -C nginx -o user= 2>/dev/null | grep -v '^root$' | sort | uniq -c | sort -rn | awk 'NR==1 {print $2}')
    if [ -z "$NGINX_USER" ]; then
        NGINX_USER=$(grep -E '^\s*user\s+' /etc/nginx/nginx.conf 2>/dev/null | awk '{print $2}' | tr -d ';')
    fi
    if [ -z "$NGINX_USER" ]; then
        # Fallback: probar los nombres de usuario de nginx mas comunes segun
        # distro (Debian/Ubuntu usan www-data; RHEL/CentOS/Fedora usan nginx).
    ...
    # Nginx necesita permiso de *traversal* (x) en cada componente de la ruta
    # hasta el dir de bundles y de *lectura* (r) sobre su contenido. Esto se
    # concede con ACLs POSIX, pero requiere root.
    #
    # Lo aplicamos con `sudo` INTERACTIVO (pide tu password). NUNCA con 'sudo -n':
    # dar NOPASSWD a setfacl seria un comodin de escalada de privilegios (setfacl
    # como root puede reescribir permisos de cualquier archivo). Con sudo interactivo
    # el administrador autoriza con su password en cada corrida -> sin backdoor.
    SETFACL_BIN=$(command -v setfacl 2>/dev/null || echo setfacl)
    if command -v getfacl >/dev/null 2>&1 && \
       getfacl "$MF_BLOCKS_DIR" 2>/dev/null | grep -q "^user:${NGINX_USER}:..x"; then
        log "Permisos de traversal (ACL) ya presentes para el usuario '$NGINX_USER'."
    else
        log "Otorgando a Nginx ('$NGINX_USER') acceso a los bundles (ACLs, con sudo)."
        log "Sin esto, Nginx daria 403/404 al servir /mf-blocks/. Te pedira tu password."
        if sudo "$SETFACL_BIN" -m "u:${NGINX_USER}:x" "$HOME" "$PROJECT_DIR" "$PROJECT_DIR/var" && \
           sudo "$SETFACL_BIN" -R -m "u:${NGINX_USER}:rX" "$MF_BLOCKS_DIR" && \
           sudo "$SETFACL_BIN" -d -m "u:${NGINX_USER}:rX" "$MF_BLOCKS_DIR"; then
            log "ACLs aplicadas para '$NGINX_USER'."
        else
            warn "No se pudieron aplicar las ACLs (¿falta el paquete 'acl' o el FS no las soporta?). Hazlo a mano:"
            echo "  sudo $SETFACL_BIN -m u:${NGINX_USER}:x \"$HOME\" \"$PROJECT_DIR\" \"$PROJECT_DIR/var\""
            echo "  sudo $SETFACL_BIN -R -m u:${NGINX_USER}:rX \"$MF_BLOCKS_DIR\""
            echo "  sudo $SETFACL_BIN -d -m u:${NGINX_USER}:rX \"$MF_BLOCKS_DIR\""
            NGINX_NEEDS_ATTENTION=true
        fi
    fi

    # Validar y recargar nginx con `sudo` INTERACTIVO (pide tu password). Antes era
    # `sudo -n` (sin password) y fallaba si no habia NOPASSWD; ahora respeta la
    # politica "sudo con password". sudo cachea la credencial, asi que el password se
    # teclea una vez. Se captura stderr porque ahi nginx reporta archivo/linea/directiva
    # exactos cuando la validacion falla.
    if nginx_t_err=$(sudo nginx -t 2>&1); then
        if reload_err=$(sudo systemctl reload nginx 2>&1); then
            log "Nginx configurado y recargado."
        else
            warn "Nginx validado pero no se pudo recargar: $reload_err"
            warn "Ejecuta manualmente: sudo systemctl reload nginx"
            NGINX_NEEDS_ATTENTION=true
        fi
    else
        warn "Error en la validacion de Nginx: $nginx_t_err"
        warn "Ejecuta manualmente: sudo nginx -t && sudo systemctl reload nginx"
        NGINX_NEEDS_ATTENTION=true
    fi
}
...
main() {
    ...
    echo ""
    if [ "$NGINX_NEEDS_ATTENTION" = true ]; then
        echo -e "${YELLOW}╔══════════════════════════════════════════════════════╗${NC}"
        echo -e "${YELLOW}║   Instalacion completada con advertencias            ║${NC}"
        echo -e "${YELLOW}╚══════════════════════════════════════════════════════╝${NC}"
        echo ""
