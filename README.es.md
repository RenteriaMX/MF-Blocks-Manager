...
│                Volto Frontend                    │
│  volto-mfblocks                                  │
│  ├── Registry de bloques (endpoint same-origin)  │
...
> **Nota Nginx:** El instalador llama a `sudo /usr/sbin/nginx -t`, `sudo /usr/bin/systemctl reload nginx` y `sudo <ruta-de-setfacl>` (para dar al usuario de los workers de Nginx permiso de traversal sobre `$HOME` sin abrirlo a todos los demas). El instalador resuelve la ruta absoluta de `setfacl` el mismo (via `command -v setfacl`) para que la llamada a `sudo` coincida con una regla de `sudoers` por ruta exacta — ejecuta `command -v setfacl` en tu servidor para encontrar esa ruta (comunmente `/usr/bin/setfacl` en Debian/Ubuntu, a veces `/usr/sbin/setfacl` en otras distros) y agrega una regla como:
>
> ```
> plone ALL=(ALL) NOPASSWD: /usr/sbin/nginx -t, /usr/bin/systemctl reload nginx, /usr/bin/setfacl
> ```
>
> Sin `NOPASSWD` para `setfacl`, el instalador imprime el comando exacto para ejecutarlo manualmente (no aborta el resto de la instalacion).
...
### Props que inyecta el host

Ademas de los props estandar de Volto, el loader pasa dos props extra a los
componentes `view` y `edit` de tu bloque para que pueda llamar a endpoints
protegidos del backend:

- `authToken` — el JWT del usuario logueado (o `null` si es anonimo).
- `apiPath` — la URL base del backend Plone (util en headless, donde el API
  vive en un origin distinto al del frontend).

```js
const View = ({ data, authToken, apiPath }) => {
  const headers = authToken ? { Authorization: `Bearer ${authToken}` } : {};
  // fetch(`${apiPath}/@mi-endpoint-protegido`, { headers })
  // ...
};
```

Ambos son aditivos y opcionales — los bloques que no los necesitan los
ignoran. Ten en cuenta que un bloque MF es JavaScript de terceros corriendo
same-origin en la pagina, asi que ya podria acceder por su cuenta al store de
Redux, cookies y localStorage; `authToken` es una conveniencia para bloques
legitimos, no un privilegio nuevo.
...
El `webpack.base` compartido emite automaticamente un `mf-manifest.json`
dentro del bundle — el contrato explicito que el backend lee al instalar para
auto-detectar el modulo expuesto y la version:

```json
{ "name": "voltoMiBloqueBlock", "module": "./block", "version": "1.0.0" }
```

Si compilas tu bundle sin `webpack.base`, incluye este archivo en la raiz del
tarball tu mismo. Los bundles sin manifest siguen funcionando: el backend cae
al parsing de `remoteEntry.js`, pero ese parsing depende del formato exacto
del output de webpack/terser y puede fallar en silencio con otras versiones.
...
- **El contenido de los bloques MF no se server-renderiza.** El pre-registro
  SSR solo evita el error "Unknown Block" para visitantes anonimos; el
  contenido real del bloque se renderiza en el cliente despues de la
  hidratacion. Si el SEO o el render sin JS del contenido importa, usa bloques
  Volto compilados nativamente para ese contenido.
- **Se asume deployment en un solo host.** Los bundles se extraen al
  filesystem local (`var/mf-blocks/`) de la instancia backend que recibe la
  subida. Un cluster ZEO multi-maquina necesitaria un filesystem compartido u
  object storage para ese directorio.
- **Frontera de confianza:** quien tenga el permiso `cmf.ManagePortal`
  (Site Administrators) puede subir bundles, es decir, ejecutar JavaScript
  arbitrario en el navegador de todos los visitantes. Es el feature
  funcionando como se diseno — otorga ese permiso en consecuencia.
- El servidor SSR necesita `curl` en su PATH para el unico fetch sincrono del
  registry en el arranque (ya listado en Requisitos).
