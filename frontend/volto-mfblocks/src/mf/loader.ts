...
function loadScript(url: string): Promise<void> {
  ...
  // MFB-3: normalizar el encoding antes de validar para que '..' codificado
  // (%2e%2e) o backslashes no evadan el filtro de traversal.
  let decoded = url;
  try {
    decoded = decodeURIComponent(url);
  } catch (e) {
    return Promise.reject(new Error(`Malformed remote URL rejected: ${url}`));
  }
  if (
    !url.startsWith('/mf-blocks/') ||
    decoded.includes('..') ||
    decoded.includes('\\')
  ) {
