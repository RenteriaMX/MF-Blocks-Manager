...
// Import profundo de un modulo interno de Volto: la ruta puede cambiar entre
// versiones (Volto 18+). Si desaparece, degradamos a un wrapper passthrough —
// se pierde el drag handle de los sub-bloques pero el editor sigue funcionando.
let DefaultEditBlockWrapper: any;
try {
  DefaultEditBlockWrapper =
    require('@plone/volto/components/manage/Blocks/Container/EditBlockWrapper').default;
} catch {
  console.warn(
    '[MF] Volto EditBlockWrapper not found at the expected path; using a passthrough wrapper.',
  );
  DefaultEditBlockWrapper = ({ children }: any) => <>{children}</>;
}
...
/**
 * Contexto del host que se inyecta a cada bloque remoto via props.
 * - authToken: JWT del usuario en el store de Volto (o null si es anonimo).
 *   El bloque lo usa para autenticar llamadas al backend:
 *     Authorization: `Bearer ${authToken}`
 * - apiPath: URL base del backend Plone (relevante en headless, donde el
 *   API no necesariamente vive en el mismo origin que el frontend).
 *
 * Se lee del store (useSelector) y no de un snapshot, para que el bloque
 * se re-renderice cuando el token se renueve.
 */
function useHostBlockProps() {
  const authToken = useSelector((s: any) => s.userSession?.token || null);
  const apiPath = config.settings?.apiPath || '';
  return { authToken, apiPath };
}
...
export function createMFBlockComponents(
  ...
  const View = (props: any) => {
    ...
    const hostProps = useHostBlockProps();
    ...
    if (blockDef?.container) {
      ...
      return <Component {...props} {...hostProps} renderColumnBlocks={renderColumnBlocks} />;
    }
